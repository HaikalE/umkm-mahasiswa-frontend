...
const UmkmActiveProjectsPage = () => {
  ...
  const fetchActiveProjects = async () => {
    try {
      ...
      const response = await fetch(`/api/umkm/active-projects?status=${filter}`);
      const data = await response.json();
      
      if (data.success) {
        setActiveProjects(data.data.projects);
      }
    } catch (error) {
  ...
  const fetchStats = async () => {
    try {
      // TODO: Replace with actual API call
      const response = await fetch('/api/umkm/active-projects/stats');
      const data = await response.json();
...
const ProjectDetailsModal = ({ project, onClose, onUpdate }) => {
  ...
  const handleApproveCompletion = async () => {
    try {
      setLoading(true);
      // TODO: Replace with actual API call
      const response = await fetch(`/api/umkm/active-projects/${project.id}/complete`, {
        method: 'POST',
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg max-w-4xl w-full max-h-[90vh] overflow-hidden">
        ...
        <div className="p-6 overflow-y-auto max-h-[60vh]">
          ...
          {activeTab === 'progress' && (
            <div>
              <p className="text-gray-600">Progress tracking akan ditampilkan di sini...</p>
            </div>
          )}

          {activeTab === 'chat' && (
            <div>
              <p className="text-gray-600">Chat dengan mahasiswa akan ditampilkan di sini...</p>
            </div>
          )}
